...
class RustRuleConfig(BaseModel):
    minimum_area_ratio: float = Field(
        default=0.02,
        ge=0,
        le=1,
    )

    medium_area_ratio: float = Field(
        default=0.08,
        ge=0,
        le=1,
    )

    high_area_ratio: float = Field(
        default=0.20,
        ge=0,
        le=1,
    )
...
class TowerTiltRuleConfig(BaseModel):
    warning_angle: float = Field(
        default=1.0,
        ge=0,
        le=90,
    )

    high_angle: float = Field(
        default=2.0,
        ge=0,
        le=90,
    )

    critical_angle: float = Field(
        default=3.0,
        ge=0,
        le=90,
    )
...
def validate_rule_config(
    ...
    adapters: dict[
        ...
        RuleEventType.FIRE: TypeAdapter(
            FireRuleConfig
        ),
        RuleEventType.SMOKE: TypeAdapter(
            SmokeRuleConfig
        ),
        RuleEventType.RUST: TypeAdapter(
            RustRuleConfig
        ),
        RuleEventType.TOWER_TILT: TypeAdapter(
            TowerTiltRuleConfig
        ),
    }

    validated = adapters[event_type].validate_python(
        config
    )
